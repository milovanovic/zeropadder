...
class ZeroPadderNative [T <: Data: Real] (val params: ZeroPadderParams[T]) extends Module {
  ...
  val dataQueue = Module(new QueueWithSyncReadMem(params.proto.cloneType, entries = params.queueDepth, flow = false, useSyncReadMem = params.useBlockRam, useBlockRam = params.useBlockRam))
